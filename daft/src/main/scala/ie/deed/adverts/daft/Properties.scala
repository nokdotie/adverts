...
object Properties {
    ...
    given JsonDecoder[ResponseListingListing] = DeriveJsonDecoder.gen[ResponseListingListing]

    case class ResponseListingListingMedia(
      images: Option[List[ResponseListingListingMediaImage]],
    )
    given JsonDecoder[ResponseListingListingMedia] = DeriveJsonDecoder.gen[ResponseListingListingMedia]


    case class ResponseListingListingMediaImage(
        size720x480: String,
    )
    given JsonDecoder[ResponseListingListingMediaImage] = DeriveJsonDecoder.gen[ResponseListingListingMediaImage]

    case class ResponseListingListingSeller(
        name: String,
        phone: Option[String]
    )
    given JsonDecoder[ResponseListingListingSeller] = DeriveJsonDecoder.gen[ResponseListingListingSeller]
    ...
    private def toRecordOption(listing: ResponseListingListing): Option[Record] = {
        val price = listing.price.filter(_.isDigit).toIntOption
        val eircode = listing.title.pipe(Eircode.regex.findFirstIn).map(_.filter(_.isLetterOrDigit))
        val phone = listing.seller.phone

        ((price, eircode, phone) match {
            case (Some(price), Some(eircode), Some(phone)) => Some((price, eircode, phone))
            case _ => None
