...
object Property {
  ...
  protected[daftie] given JsonDecoder[ResponsePageProps] =
  ...
  protected[daftie] given JsonDecoder[ResponsePagePropsListing] =
    DeriveJsonDecoder.gen[ResponsePagePropsListing]

  protected[daftie] case class ResponsePagePropsListingMedia(
      images: Option[List[ResponsePagePropsListingMediaImage]]
  )
  protected[daftie] given JsonDecoder[ResponsePagePropsListingMedia] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingMedia]

  protected[daftie] case class ResponsePagePropsListingMediaImage(
      size1440x960: String
  )
  protected[daftie] given JsonDecoder[ResponsePagePropsListingMediaImage] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingMediaImage]
  ...
  protected[daftie] given JsonDecoder[ResponsePagePropsListingBer] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingBer]

  protected[daftie] case class ResponsePagePropsListingFloorArea(
      value: String,
      unit: String
  )
  protected[daftie] given JsonDecoder[ResponsePagePropsListingFloorArea] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingFloorArea]

  protected[daftie] case class ResponsePagePropsListingPoint(
      coordinates: List[BigDecimal]
  )
  protected[daftie] given JsonDecoder[ResponsePagePropsListingPoint] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingPoint]

  protected[daftie] case class ResponsePagePropsListingSeller(
      licenceNumber: Option[String]
  )
  protected[daftie] given JsonDecoder[ResponsePagePropsListingSeller] =
    DeriveJsonDecoder.gen[ResponsePagePropsListingSeller]
  ...
  private def size(response: Response): Option[Area] = {
    val value =
      response.pageProps.listing.floorArea.map { _.value }.map { BigDecimal(_) }
  ...
  private def advertiser(
    ...
    response.pageProps.listing.seller.licenceNumber.fold(ZIO.succeed(None)) {
      AdvertiserStore.getByPropertyServicesRegulatoryAuthorityLicenceNumber(_)
    }
  ...
  protected[daftie] def toDaftIeAdvert(
    ...
    DaftIeAdvert(
      ...
      imageUrls =
        response.pageProps.listing.media.images.getOrElse(List.empty).map {
          _.size1440x960
        },
      size = size(response),
