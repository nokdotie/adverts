...
object Properties {
    ...
    private given JsonDecoder[ResponseSearchResult] = DeriveJsonDecoder.gen[ResponseSearchResult]
    ...
    private given JsonDecoder[ResponseSearchResultNegotiator] = DeriveJsonDecoder.gen[ResponseSearchResultNegotiator]
    ...
    private def toRecordOption(searchResul: ResponseSearchResult): Option[Record] = {
        val price = searchResul.PriceAsString.getOrElse("").filter(_.isDigit).toIntOption
        val eircode = searchResul.DisplayAddress.pipe(Eircode.regex.findFirstIn).map(_.filter(_.isLetterOrDigit))

        ((price, eircode, searchResul.Negotiator) match {
            case (Some(price), Some(eircode), Some(negotiator)) if negotiator.Email.nonEmpty || negotiator.Phone.nonEmpty => Some((price, eircode, negotiator))
            case _ => None
